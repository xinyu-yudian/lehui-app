...
<template>
	<view class="draw-money-wrap u-p-b-30">
		...
		<view class="draw-card">
			...
			<view class="give_box" v-if="giveNum>0">
				本次充值赠送{{giveNum}}元
			</view>
			<!-- 快捷充值 -->
			<view class="top-up-content u-p-b-30" v-if="initRecharge.moneys.length">
				<view class="card-title mt10">快捷充值</view>
		...
		watch: {										
			money(newVal, old){
				for(var i=0;i<this.giveList.length;i++){
					if(newVal==this.giveList[i]['rechargenum']){
						this.giveNum = this.giveList[i]['givenum']
						return
					}
				}
				this.giveNum = 0
