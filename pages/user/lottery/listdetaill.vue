	...
	<view class="detail-wrap ">
		...
		<view class="detail_box" v-else>
			<view class="detail-content">
				<!-- 轮播 -->
				<u-swiper :height="550" borderRadius="0" :list="goodsInfo.images" indicator-pos="bottomRight" mode="number" :interval="3000" @click="onGoodsSwiper"></u-swiper>
				...
				<view class="u-m-b-10 title-box u-p-20 u-skeleton-fillet">
					<view class="goods-title u-m-b-10 u-ellipsis-2" style="border-bottom:1px solid #F0F8FF"><span style="margin-left:5px;">{{ goodsInfo.title }}</span></view>
					<view class="sub-title u-ellipsis-2" style="border-bottom:1px solid #F0F8FF">{{ goodsInfo.subtitle }}</view>
					<view style="text-index:50px;">
						<div style="border-bottom:1px solid #F0F8FF"><span style="margin-left:5px;">{{ goodsInfo.content}}</span></div>
					</view>
					<view style="text-index:50px; border-bottom:1px solid #F0F8FF;">
						<div class="acsm" style="background-color:#F5F5DC; margin-top:8rpx; height:auto; width:700rpx; bottom:13px; text-indent: 20px; padding:5px; border-radius:5%;">
							{{ goodsInfo.introduce}}
						</div>
					</view>
				</view>
				...
				<view class="detail-foot_box safe-area-inset-bottom  u-flex u-col-ceter u-row-around">
					...
					<view class="detail-right">
						...
						<view v-if="detailType !== 'score'">
							<!-- 正常按钮 -->
							<view class="detail-btn-box u-flex u-row-around" v-if="goodsInfo.is_join == 2">
								...
								<view class="tools-item u-flex-col u-row-center u-col-center" @tap="onShare">
									...
									<u-icon name="share-fill" class="tool-img" color="#464958" size="45"></u-icon>
									<text class="tool-title">分享</text>
								</view>

								<button class=" u-reset-button tool-btn add-btn add-goods-cart" @click="join_lottery">参与抽奖</button>
								<!-- <button class=" u-reset-button tool-btn pay-btn" @tap="goPay">立即购买</button> -->
							</view>

							<view class="detail-btn-box u-flex u-row-around" v-if="goodsInfo.is_join == 1  ">

								<button class=" u-reset-button tool-btn add-btn add-goods-cart" @tap="addCart" v-if="goodsInfo.ifstart == 0">等待开奖</button>
								<button class=" u-reset-button tool-btn add-btn add-goods-cart" v-if="goodsInfo.ifstart == 1">已开奖</button>

								<!-- <button class=" u-reset-button tool-btn pay-btn" @tap="goPay">立即购买</button> -->
							</view>
							<view class="detail-btn-box u-flex u-row-around" v-if="goodsInfo.is_join == 3">
								<button class=" u-reset-button tool-btn add-btn add-goods-cart">已结束</button>
								<!-- <button class=" u-reset-button tool-btn pay-btn" @tap="goPay">立即购买</button> -->
							</view>
							<!-- 活动按钮 -->
							<view class="detail-btn-box u-row-around" v-if="activityRules.status && goodsInfo.activity_type">
								...
								<block v-else>
									...
									<view class="u-flex groupon-btn-box" v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon'">
										<button class="tool-btn u-reset-button add-btn u-flex-col u-row-center u-col-center" @tap="payGroupon" v-if="goodsInfo.activity.rules.is_alone === '1'">
											<text class="price font-OPPOSANS">￥{{ goodsInfo.price }}</text>
											<text class="price-title">单独购买</text>
										</button>
										<button class=" tool-btn u-reset-button groupon-btn u-flex-col u-row-center u-col-center" :style="goodsInfo.activity.rules.is_alone === '0' ? 'width:400rpx' : ''" @tap="payGroupon('groupon')">
			...
			<!-- 规格弹窗 -->
			<shopro-sku v-if="showSku && goodsInfo.id" v-model="showSku" :goodsInfo="goodsInfo" :activityRules="activityRules" :buyType="goodsInfo.activity_type == 'seckill' || detailType === 'score' ? 'buy' : buyType" :grouponBuyType="grouponBuyType" :goodsType="detailType === 'score' ? 'score' : 'goods'" @changeType="changeType" @getSkuText="getSkuText"></shopro-sku>

			<!-- 分享组件 -->
			<shopro-share v-model="showShare" :showBtnPoster="false" :posterInfo="goodsInfo" :posterType="'goods'"></shopro-share>
			<!-- 登录 -->
	...
	export default {
		...
		data( ) {
			return {
				...
				videopath: 'https://v.youku.com/v_show/id_XNTgzNTQ5OTIzMg==.html?spm=a2h0c.8166622.PhoneSokuUgc_15.dtitle'
			};
		},
		// 是否登录
		computed: {
		...
		methods: {
			...
			goCart( ) {
				this.$Router.replaceAll( '/pages/index/cart' );
			},
			// 选项卡
			onTab( id ) {
				this.tabCurrent = id;
			...
			join_lottery( ) {
				...
				wx.requestSubscribeMessage( {
					tmplIds: [ 'c0_5sKztOy87-17ju_bObKl8UtK8J7WeQ4S0qD6XywA' ],
					success: ( res ) => {
						return new Promise( ( resolve, reject ) => {
							that.$http(
								'lottery.joinLottery', {
