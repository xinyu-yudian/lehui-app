	...
	<view>
		...
		<view class="normal-price-box" v-if="type !== 'score' && !detail.activity">
			...
			<view class="u-flex u-row-left price-bottom-box">
				<view class="u-flex">
					<view class="sold font-OPPOSANS">
						奖品：<text v-for="award in detail.award_list" :key="award.id">{{award.title}}</text>
					</view>
				</view>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;">
				<text style="">开奖时间：</text> <text>{{ detail.lotterytime | formatDate }}</text>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;">
				<text style="">开奖方式：</text> <text>{{ detail.open_text}}</text>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;" v-if="detail.typedata==3">
				<text style="">开奖人数：</text> <text>{{ detail.user_count}}人</text>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;">
				<text style="">已参加：</text> <text>{{ detail.users }}人</text>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;" v-if="detail.win == 1">
				<text style="">开奖结果：</text> <text>恭喜你中奖了！奖品： {{detail.win_prize.title}}</text>
			</view>
			<view style="color: #f0f0f0;font-size: 22rpx;margin-top: 10rpx;" v-if="detail.win == 0">
				<text style="">开奖结果：</text> <text>您未中奖，请再接再厉</text>
			</view>
		...
		<view class="groupon-price-box" v-if="detail.activity && detail.activity.type === 'groupon'">
			<view class="u-flex u-col-bottom">
				...
				<text
					class="price font-OPPOSANS">{{ detail.activity_type === 'groupon' ? detail.groupon_price : detail.price }}</text>
			...
			<view class="u-flex u-row-between price-bottom-box">
				...
				<view class="count-down font-OPPOSANS" v-show="activityRules.status !== 'end'">
					...
					{{ activityRules.countDownTime.h || '00' }} : {{ activityRules.countDownTime.m || '00' }} :
					{{ activityRules.countDownTime.s || '00' }}
				</view>
		...
		<!-- 秒杀商品 -->
		<view class="seckill-price-box " :class="activityRules.status !== 'end' ? 'seckill-bg' : 'seckilled-bg'"
			v-if="detail.activity && detail.activity.type === 'seckill'">
				...
				<view class="express">
					<view class="u-flex">
						<view style="width:100rpx;" v-show="activityRules.status !== 'end'">
							<u-line-progress height="14" :show-percent="false" :percent="percent"
								inactive-color="#a1071a" active-color="#fff"></u-line-progress>
						</view>
	...
	export default {
		...
		computed: {
			...
			percent() {
				let unit = 0;
				if (this.detail.id) {
					unit = this.detail.stock + this.detail.sales > 0 ? ((this.detail.sales / (this.detail.sales + this
						.detail.stock)) * 100).toFixed(2) : 0;
				}
				return Number(unit);
			}
		},
