...
<template>
	<view class="order-wrap">
		<view class="head-nav order-type">
			<view class="order-switch">
				<view class="head-tabs">
					<view :class="navIndex==1?'activite':''" class="head-tabs-item tables-first" @click="checkIndex(1)">厨师上门</view>
					<view :class="navIndex==2?'activite':''" class="head-tabs-item tables-second" @click="checkIndex(2)">外送</view>
				</view>
				<view class="head_box" v-if="orderPre.need_address" @tap="jump('/pages/user/address/list', { from: 'order' })">
					<view class="add-address-box u-flex u-flex-1" v-if="!addressId">
						<view class="box-bg u-p-30 u-flex-1 u-flex u-row-between">
							<text class="select-notice emptyAddress">选择地址</text>
				...
				<view class="u-m-b-10 choose-time">
					<!-- 配送时间 -->
					<view class="logistic order-times item-list u-flex u-row-between" @tap="onServiceTime()">
						<view class="item-title">
							服务时间
						</view>
						<view class="u-flex">
							<view class="detail">{{ checkDayCur==-1?'请选择':checkTime['day'][checkDayCur].title }}{{ checkTimeCur==-1?'':checkTime['time'][checkTimeCur] }}</view>
							<text class="u-iconfont uicon-arrow-right" style="color: #bfbfbf;"></text>
							<!-- <u-action-sheet :actions="timeList" :closeOnClickOverlay="true" :closeOnClickAction="true"  :title="timeTitle" :show="timeShow"></u-action-sheet> -->
						</view>
					</view>
					<view class="service-tips"  v-if="navIndex == 1">您选择C厨上门烹饪，请保持电话畅通~</view>
				</view>
			</view>
		</view>
		<!-- 地址栏 -->
		<view class="u-m-b-10 order-details">
			<!-- 确认订单商品卡片 -->
			...
			<block v-if="perGoodsList.length">
				...
				<view class="price-box  u-flex u-row-between item-list">
					<view class="item-title u-flex u-col-center">
						<view class="u-m-r-10">厨师服务费</view>
						<view class="activity-title" v-if="Number(orderPre.cook_service_discount_money) > 0">套餐减￥
							{{ orderPre.cook_service_discount_money }}</view>
					</view>
					<view class="u-flex">
						<text class="origin-price u-m-r-10"
							v-if="Number(orderPre.cook_service_discount_money) > 0">-￥{{ orderPre.cook_service_amount }}</text>
						<text
							class="price">￥{{ Number(orderPre.cook_service_amount) - Number(orderPre.cook_service_discount_money) || '0.00' }}</text>
					</view>
				</view>
			...
			<view class="foot-box u-flex u-row-between safe-area-inset-bottom">
				<view class="u-flex">
					...
					<view class="all-money">
						<!-- <text>合计：</text> -->
						<view class="price">￥<text>{{ orderPre.total_fee || '0.00' }}</text></view>
						<view class="discount-num">
							<!--活动优惠+配送优惠+厨师优惠+优惠券-->
							已优惠{{ (Number(orderPre.activity_discount_money) + Number(orderPre.dispatch_discount_money) + Number(orderPre.cook_service_discount_money)) + Number(orderPre.coupen_fee) || '0.00' }}
						</view>
						...
						<view class="express-content u-flex">
							<view class="time-box">
								...
								<view class="box-content u-flex" @tap="checkExpressTime('store')">
									<text
										class="box-text">{{ checkDayCur==-1?'请选择':checkTime['day'][checkDayCur].title }}{{ checkTimeCur==-1?'':checkTime['time'][checkTimeCur] }}</text>
									<text class="u-iconfont uicon-arrow-right" style="color: #999"></text>
								</view>
							</view>
							<view class="box-line"></view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				is_taocan:0,//是否购买有套餐 1是 0否
				cookMoney: 58,//厨师上门最低金额
				navIndex: 2,// 厨师上门和外送切换
		...
		methods: {
			...
			checkIndex(type){
				...
				if(that.is_taocan != 1 && type == 1 && Number(this.orderPre.goods_amount) < Number(this.cookMoney)){
					let diff = Number(this.cookMoney) - Number(this.orderPre.goods_amount);
					uni.showModal({
					 	title:'提示信息',
					 	content: '厨师上门需要消费满'+this.cookMoney+'元，还差'+diff.toFixed(2)+'元',
					 	confirmColor:'#001A88',
					 	cancelText:'取消',
					 	confirmText:'继续购买',
					 	success: (res) => {
			...
			init() {
				...
				return Promise.all([this.getDefaultAddress(), this.getCoupons()]).then(() => {
					uni.hideLoading();
				});
			},
			jump(path, parmas) {
			...
			initDate() {
				...
				let timeArr = [];
				let item =[];
				if(this.checkType == '店家配送'){
					item = this.workTime.goods;
				}else if(this.checkType == '厨师上门'){
					item = this.workTime.master;
				}
				if(item.length>0){
					item.forEach(i => {
						timeArr.push(i);
			...
			getWorkTime(){
				let that = this;
				that.$http('order.time',{},'',false).then(res => {
					if(res.code ==1){
						if(res.data){
							that.workTime = res.data;
							that.workTime.goods = Object.values(res.data.goods);
			...
			getPre() {
				...
				that.$http('order.pre', {
					...
					is_cook: that.navIndex,
				}).then(res => {
					if (res.data) {
						that.orderPre = res.data;
						that.cookMoney = res.data.cook_money;
						that.is_taocan = res.data.is_taocan;
			...
			subOrder() {
				let that = this;
				if(that.checkDayCur == -1){
					that.$u.toast('请选择配送时间');
					return;
				}
			...
			// 更改提交数据
			changeGoodsList() {
				console.log('this.checkDayCur');
				console.log(this.checkDayCur);
				console.log('this.checkTimeCur');
				console.log(this.checkTimeCur);
